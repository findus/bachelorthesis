...
Bestandteile zerlegt.\cite[S. 257ff]{Bruegge_2004}\par % Sinngemäß, kein wörtliches Zitat.
...
Equipments benötigten Klassen.\par
...
Das Paket Input enthält die Klassen zur Verarbeitung von Benutzereingaben. Unter anderem werden darin variable
Tastenbelegungen und die Erzeugung von Steuerbefehlen implementiert.\par

Im Paket GameController befinden sich schließlich alle Klassen, die zur Initialisierung des Systems, Verwaltung von
Programmzuständen und Steuerung von Menüs benötigt werden. Außerdem wird hier auch die Logik, also der eigentliche
Ablauf, des Spiels umgesetzt.\par

Eine Übersicht über die Paketstruktur von Cydonia gibt auch Abbildung \ref{figure:global_control_flow}.
...
resultierende Reaktion, nämlich dem Player-Paket den Befehl zu geben, das aktuelle Equipment des Spielers zu ermitteln
und dessen Benutzung anzustoßen, wodurch der Befehl im Equipment-Paket landet. Dort ist die auszuführende Aktion für das
Pick'n'Place-Equip\-ment implementiert (Rückmeldungen für den Spieler, Manipulation der Spielwelt usw.).
Das Equipment-Paket meldet folglich dem GameController-Paket die gewünschte Manipulation der Spielwelt, nämlich das
Platzieren des im Equipment vorrätigen Cubes an einer bestimmten Position. Letzt\-end\-lich wird die Platzierung des
Cubes im Paket GameWorld ausgeführt und der Spieler kann den neuen Cube sehen. Dieser beispielhafte Ablauf kann auch
...
Dazu ist ein sog. Server nötig, eine Instanz des Spiels, die die Autorität über die Geschehnisse in der virtuellen Welt
